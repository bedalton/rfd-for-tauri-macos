...
impl Panel {
    ...
    pub fn set_path(&self, path: &Path) {
        let default_path: PathBuf = path.components().collect();
        if default_path.is_file() || !default_path.exists() {
            if let (Some(parent), Some(file_name)) = (default_path.parent(), default_path.file_name()) {
                if parent.components().count() > 0 {
                    self.set_directory(&parent.to_string_lossy());
                }
                self.set_file_name(&file_name.to_string_lossy());
            } else {
                self.set_directory(&default_path.to_string_lossy());
            }
        } else {
            self.set_directory(&default_path.to_string_lossy())
        }
    ...
    pub fn set_directory(&self, directory: &str) {
        unsafe {
            let url = NSURL::file_url_with_path(directory, true);
